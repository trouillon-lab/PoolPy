...
def _html_to_text(msg_html: str):
    message = re.sub(r'</?b>', '', msg_html)
...
def _build_text_message(file_name: str, readout_list: list, diff_deco: int, n_compounds: int, n_pools: int,
    ...
    msg = f"Processing file <b>{file_name}</b> with max. <b>{diff_deco}</b> positive samples and readout: <br>"
    ...
    if n_compounds_f < 2:
        if n_compounds_f == 1:
            ...
            msg += f'<span style="color: #2ecc40;"><b>Sample: {decoded[0]}</b></span><br>'
        else:
    ...
    if decoded_type == "error":
        ...
        if isinstance(decoded, list) and len(decoded) == 1 and isinstance(decoded[0], list) and len(decoded[0]) == 1:
            msg += '<span style="color: #2ecc40;"><b>A single positive sample was found:</span></b><br>'
            msg += f'<span style="color: #2ecc40;"><b>Sample: {decoded[0][0]}</b></span><br>'
        else:
            msg += '<span style="color: #2ecc40;"><b>A single possible combination of positive samples was found. The positive samples are:</span></b><br>'
            combo = decoded[0] if isinstance(decoded, list) and len(decoded) == 1 else decoded
            msg += f'<span style="color: #2ecc40;"><b>Samples: {", ".join(map(str, combo))}</b></span><br>'
    else:
