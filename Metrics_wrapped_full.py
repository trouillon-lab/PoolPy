	...
	for n_val in sorted(n_values):
		...
		for diff_folder in diff_folders:
			...
			for wa_file in wa_files:
				...
				metrics = compute_full_metrics(n_val, diff_val, wa)
			...
			try:
				hier_metrics = calculate_full_metrics_hierarchical_fast(n_val, diff_val, checks=65)
				hier_row = {
					'N': n_val,
					'diff': diff_val,
					...
					'Mean positive pools':hier_metrics[6],
        			'Mean negative pools': hier_metrics[7],
				}
