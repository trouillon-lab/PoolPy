...
def sweep_fly_summary(start, stop, step, start_diff, stop_diff, step_diff, directory, folder_N=True, folder_diff=True, checks_hierarchical=1000):
    """
...
def fly_summary(n_compounds:int,  differentiate:int, checks_hierarchical=65, **kwargs):
    ...
    MD=np.max([4, int(np.log(n_compounds))])
    # Multidimensional methods (2D, 3D, 4D)
    for n_dims in np.arange(2, MD+1):
        ls_dims=get_params_multidims(n_compounds, n_dims)
        arr_dims=np.array(ls_dims)
        method_name = f'multidim-{n_dims}' if n_dims>2 else 'Matrix'
        ...
        MEE=0 if differentiate==1 else np.min([differentiate**n_dims, generalized_factorial(differentiate,  n_dims), n_compounds ])
...
def rand_sweep_diff(n_compounds, max_diff, Npath, **kwargs):
