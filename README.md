...
`--readout`: A string directly containing the readout, or containing a path to a .csv file of the readout. The readout should be provided as a comma-delimited list of positive pools (e.g., for five pools with pool 2 and 3 being positive: `0,0,1,1,0`; or by simply listing the positive pools: `2,3`). Numbering of pools should follow the format given in the design file generated by PoolPy (0-based numbering).

&nbsp;

## 🧪 Example: From Design to Decoding

This example demonstrates how to generate a pooling plan for 100 samples, run your tests, and decode the results using the code locally (all of which is also possible on the web app).
