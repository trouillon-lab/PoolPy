...
app_ui = ui.page_fluid(
    ...
    ui.navset_tab(
        ui.nav_panel("Pooling Methods Comparison",
            ...
            ui.row(
                ui.column(4),
                ui.column(2, ui.input_numeric("n_samp", "Number of Samples:", value=100)),
                ui.column(1),
            ...
            ui.row(
                ui.column(4),
                ui.column(2, ui.input_numeric("sensitivity", "Test sensitivity (optional):", value=1, min=0, max=1, step=0.01)),
                ui.column(1),
                ui.column(2, ui.input_numeric("specificity", "Test specificity (optional):", value=1, min=0, max=1, step=0.01)),
                ui.column(3),
            ),
            ui.row(
                ui.column(
            ...
            ui.panel_conditional(
                ...
                ui.div(
                    ui.div(
                        ui.HTML('<div style="text-align: center; font-size: 18px; margin-bottom: 8px;">Single sample testing</div>'),
                        ui.output_ui("single_sample_table"),
...
# Add a new output to render the colored summary table in the server function, not in the UI definition
            ...
            ui.tags.script('''
                ...
                document.getElementById('sensitivity').addEventListener('keydown', function(event) {
                    if (event.key === 'Enter') {
                        event.preventDefault();
                        document.getElementById('submit').click();
                    }
                });
                document.getElementById('specificity').addEventListener('keydown', function(event) {
                    if (event.key === 'Enter') {
                        event.preventDefault();
                        document.getElementById('submit').click();
                    }
        ...
        ui.nav_panel("Prevalence",
                ...
                ui.div(
                    ui.row(
                        ...
                        ui.column(1),
                        ui.column(2,
                            ui.input_numeric("steps_prev", "Maximum numbper of hierarchical steps (optional)", value=100),
                        ),
                        ui.column(2),
                    ),
                ...
                ui.div(
                    ui.input_action_button("BG2_submit", "Enter", style="display: block; margin: 20px auto 0 auto;"),
                    style="text-align: center;"
                ),
                ui.tags.script('''
                    document.addEventListener('DOMContentLoaded', function() {
                        ["BG2_prev_prevalence", "steps_prev"].forEach(function(id) {
                            var el = document.getElementById(id);
                            if (el) {
                                el.addEventListener('keydown', function(event) {
                                    if (event.key === 'Enter') {
                                        event.preventDefault();
                                        var btn = document.getElementById('BG2_submit');
                                        if (btn) btn.click();
                                    }
                                });
                            }
                ...
                '''),
                ui.div(
                    ui.output_ui("BG2_result_display"),
                    style="display: flex; justify-content: center; margin-top: 24px; margin-bottom: 8px;"
                ),
                ui.div(style="height: 36px;"),
                ui.hr(style="border: none; border-top: 2px solid #bdbdbd; width: 70%; margin: 0 auto;"),
                ui.div(style="height: 36px;"),
                ui.div(
                    ui.div(
        ...
        ui.nav_panel("About",
            ui.div(
                ui.output_image("logo_img_about", height="0px"),
                style="display: block; padding: 0; margin: 0; text-align: center;"
            ),
            ui.h2("About PoolPy", style="text-align: center; margin-top: 150px;"),
            ui.div(
                ui.div(
                    ui.p(
                        "PoolPy is an application for optimizing pooling designs to increase throughput and reduce costs in diagnostic testing.",
                        style="margin-bottom: 16px;"
                    ),
                    ui.p(
                        "It provides a practical interface to compare and select pooling strategies under real experimental constraints, "
                        "including expected prevalence, turnaround time, assay cost, and signal dilution. The goal is to make combinatorial "
                        "group testing easier to use in both standard screening workflows and more complex large-scale profiling settings.",
                        style="margin-bottom: 16px;"
                    ),
                    ui.p(
                        "The accompanying paper presents PoolPy as a unified framework for designing and selecting optimal group testing "
                        "strategies across ten different methods according to user-defined constraints. By precomputing more than 10,000 "
                        "designs and exposing them through a web interface, PoolPy helps users navigate key trade-offs such as test efficiency, "
                        "pool size, cost, and practical applicability for specific use cases.",
                        style="margin-bottom: 16px;"
                    ),
                    ui.p(
                        "Overall, the work shows that no single pooling strategy is universally optimal. Instead, the best design depends on "
                        "the balance between throughput, robustness, dilution effects, and experimental limitations in a given application.",
                        style="margin-bottom: 16px;"
                    ),
                    ui.p(
                        "All precomputed pooling designs and associated performance metrics are publicly available on Zenodo. "
                        "This repository contains the precomputed results used by PoolPy, making it easier to inspect, reuse, "
                        "and reference the design space explored by the application.",
                    ...
                    ui.p(
                        ...
                        ui.a(
                            "GitHub repository",
                            href="https://github.com/trouillon-lab/PoolPy",
                            style="text-decoration: underline;",
                            target="_blank",
                            rel="noopener noreferrer"
                        ),
...
def process_metrics_data(metrics_data, sensitivity=1, specificity=1):
    ...
    try:
        ...
        metrics_data_base = metrics_data.copy()
        
        # Apply pooling characteristic formula: pooling_char = 1 - (1 - test_char)^n_tests
        if 'Pooling sensitivity' in metrics_data.columns:
            metrics_data['Pooling sensitivity'] = (sensitivity ** metrics_data_base['Pooling sensitivity']).round(2)
        if 'Pooling specificity' in metrics_data.columns:
            metrics_data['Pooling specificity'] = (specificity ** metrics_data_base['Pooling specificity']).round(2)
        ...
        extra_cols = [c for c in ['Max experiments per sample', 'Pooling sensitivity', 'Pooling specificity'] if c in metrics_data.columns]
        metrics_data = metrics_data[['Pooling strategy', 'Mean experiments', 'Mean steps', 'N pools', 'Max samples per pool', 'Percentage check', 'Mean extra experiments'] + extra_cols]
...
def ET_BG2_Gen_opt_brute_MD(N,D,p):
    ...
    rg=np.arange(N+1)
    for i in rg:
        probs.append(scipy.stats.binom.pmf(i,NS,p))
...
def server(input, output, session):
  ...
  # For Prevalence tab: track last submitted values
    ...
    output.prev_BG_layers = reactive.Value(10)
    output.BG2_result_hierarchical = reactive.Value(None)
    output.BG2_result_md = reactive.Value(None)
    output.prev_BG_prevalence = reactive.Value(0.01)
    ...
    output.last_submitted_sensitivity = reactive.Value(1)
    output.last_submitted_specificity = reactive.Value(1)
    ...
    def _():
        ...
        output.last_submitted_sensitivity.set(sensitivity)
        output.last_submitted_specificity.set(specificity)
        # Display computing message
        #print(n_samp)
        n_samp=output.last_submitted_n_samp.get()
        differentiate=output.last_submitted_differentiate.get()
        sensitivity=output.last_submitted_sensitivity.get()
        specificity=output.last_submitted_specificity.get()
        ...
        if differentiate > n_samp:
            ...
            if n_samp * differentiate * differentiate > MAX_PROD:
                ...
                fly_table = process_metrics_data(fly_table, sensitivity=sensitivity, specificity=specificity)
            ...
            try:
                ...
                metrics_data = process_metrics_data(metrics_data, sensitivity=sensitivity, specificity=specificity)
                ...
                if 'Mean experiments' in copio.columns and 'Mean steps' in copio.columns:
                    ...
                    long_exps=f'If you mainly need to <b>reduce test number</b>, the best method might be the <b>{copio.iloc[0,0]}</b>.<br>'
                ...
                if 'Mean experiments' in copio.columns and 'Mean steps' in copio.columns:
                    ...
                    expensive_exps=f'If you need results <b>quickly</b>, the best method might be the <b>{copio.iloc[0,0]}</b>.<br>'
                ...
                if 'Mean experiments' in copio.columns and 'Max samples per pool' in copio.columns:
                    ...
                    low_pools=f'If you <b>cannot pool many samples</b> together, the best method might be the <b>{copio.iloc[0,0]}</b>.<br>'
    ...
    def _():
        n_steps = input.steps_prev()
        p = input.BG2_prev_prevalence()
        output.prev_BG_layers.set(n_steps)
        output.prev_BG_prevalence.set(p)

        # Compute optimal hierarchical strategy
        try:
            hier_result = brute_better_BG2(p, n_steps)
            output.BG2_result_hierarchical.set(hier_result)
        except Exception:
            output.BG2_result_hierarchical.set(None)

        # Compute optimal multidimensional strategy (cap D_max to avoid heavy computation)
        try:
            d_max = min(n_steps, 8)
            md_result = brute_BG2_Gen_MD(p, d_max)
            output.BG2_result_md.set(md_result)
        except Exception:
            output.BG2_result_md.set(None)
    ...
    def build_fixed_width_table(df, rows_html, col_width_px=COLUMN_WIDTH, font_size_px=16, padding="8px 12px", table_id=None):
        table_style = (
        ...
        if table_id:
            sort_header_style = header_style + " cursor:pointer; user-select:none;"
            ths = "".join(
                f'<th style="{sort_header_style}" onclick="poolpySortTable(\'{table_id}\',{i})">'
                f'{col}&nbsp;<span id="{table_id}-arrow-{i}" style="font-size:10px;">&#8645;</span></th>'
                for i, col in enumerate(df.columns)
            )
            header = f'<tr>{ths}</tr>'
            id_attr = f' id="{table_id}"'
        else:
            header = "<tr>" + "".join([f'<th style="{header_style}">{col}</th>' for col in df.columns]) + "</tr>"
            id_attr = ""
        table_html = f'<table{id_attr} style="{table_style}">{colgroup}<thead>{header}</thead><tbody>{"".join(rows_html)}</tbody></table>'
        if table_id:
            table_html += f"""<script>
(function(){{
  if(window._poolpySortInit) return;
  window._poolpySortInit = true;
  window._poolpySortState = {{}};
  window.poolpySortTable = function(tid, col) {{
    var tbl = document.getElementById(tid);
    if(!tbl) return;
    var tbody = tbl.querySelector('tbody');
    var rows = Array.from(tbody.rows);
    var state = window._poolpySortState;
    var asc = !(state[tid+'_'+col] === 'asc');
    state[tid+'_'+col] = asc ? 'asc' : 'desc';
    ...
    tbl.querySelectorAll('thead th span[id]').forEach(function(sp){{ sp.innerHTML='&#8645;'; }});
    var sp = document.getElementById(tid+'-arrow-'+col);
    if(sp) sp.innerHTML = asc ? '&#9650;' : '&#9660;';
  }};
}})();
</script>"""
        return table_html
    ...
    @output
    @render.ui
    def BG2_result_display():
        hier = output.BG2_result_hierarchical.get()
        md = output.BG2_result_md.get()
        if hier is None and md is None:
            return ui.HTML("")

        p = output.prev_BG_prevalence.get()
        card_style = (
            "flex: 1 1 320px; "
            "max-width: 420px; "
            "min-width: 280px; "
        ...
        if hier is not None:
            try:
                ...
                total_pool = int(np.prod(fv0))
                html += (
                    f'<div style="{card_style} border: 1px solid #74c476; background: #f4fff4;">'
                    '<div style="font-size: 18px; font-weight: bold; margin-bottom: 8px;">'
                    'Optimal Hierarchical Strategy</div>'
                    f'<div>Split factors at each level: <b>{splits}</b></div>'
                    f'<div>Initial pool size: <b>{total_pool}</b> samples</div>'
                    '<div style="margin-top: 6px; color: #555; font-size: 14px;">'
                    'At each step, positive pools are subdivided by the next factor until individual testing.</div>'
                    f'<div style="margin-top: 10px;">Expected tests per sample: '
                    f'<b style="color: #228B22;">{e0:.4f}</b> '
                    f'(vs. 1.0 for individual testing)</div>'
                    '</div>'
                )
            except Exception:
                pass
        ...
        if md is not None:
            try:
                ...
                total_pool_md = N ** D
                html += (
                    f'<div style="{card_style} border: 1px solid #6baed6; background: #f4f8ff;">'
                    '<div style="font-size: 18px; font-weight: bold; margin-bottom: 8px;">'
                    'Optimal Multidimensional Strategy</div>'
                    f'<div>Dimensions: <b>{D}</b></div>'
                    f'<div>Pool size per dimension: <b>{N}</b></div>'
                    f'<div>Total pool size: <b>{total_pool_md}</b> samples</div>'
                    '<div style="margin-top: 6px; color: #555; font-size: 14px;">'
                    f'Arrange {total_pool_md} samples in a {D}-dimensional {N}\u00d7... matrix; '
                    'each coordinate defines one pool.</div>'
                    f'<div style="margin-top: 10px;">Expected tests per sample: '
                    f'<b style="color: #1a6fb5;">{ET:.4f}</b> '
                    f'(vs. 1.0 for individual testing)</div>'
                    '</div>'
                )
            except Exception:
...
# On the fly downloads:
    ...
    def fly_summary_colored_table():
        ...
        html = build_fixed_width_table(df, rows_html, table_id="fly-summary-table")
        return ui.HTML(html)

    @output
    @render.ui
    def single_sample_table():
        df_ref = output.summary_table.get()
        ...
        # Build single-sample row matching columns of the main table
        row = {
            'Pooling strategy': 'Single sample',
            'Mean experiments': n_samp,
            'Mean steps': 1,
            'N pools': n_samp,
            'Max samples per pool': 1,
            'Percentage check': 0,
            'Mean extra experiments': 0,
        }
        NT=output.last_submitted_differentiate.get()
        NF=n_samp-NT
        if 'Max experiments per sample' in df_ref.columns:
            row['Max experiments per sample'] = 1
        if 'Pooling sensitivity' in df_ref.columns:
            row['Pooling sensitivity'] = round(sensitivity ** NT, 2)
        if 'Pooling specificity' in df_ref.columns:
            row['Pooling specificity'] = round(specificity ** NF, 2)

        # Only keep columns that exist in main table, in the same order
        cols = [c for c in df_ref.columns if c in row]
        df_single = pd.DataFrame([{c: row[c] for c in cols}])

        # Rename sensitivity/specificity columns to 'Experiment …' for this table
        df_single = df_single.rename(columns={
            'Pooling sensitivity': 'Single exp. sensitivity',
            'Pooling specificity': 'Single exp. specificity',
        })

        base_cell_style = fixed_width_cell_style()
        row_html = '<tr>' + ''.join(f'<td style="{base_cell_style}">{v}</td>' for v in df_single.iloc[0]) + '</tr>'
        html = build_fixed_width_table(df_single, [row_html])
        return ui.HTML(html)
    ...
    def summary_colored_table():
        ...
        for i, row in df.iterrows():
            ...
            row_html += '</tr>'
            rows_html.append(row_html)
        html = build_fixed_width_table(df, rows_html, table_id="summary-table")
        return ui.HTML(html)
    ...
    def prevalence_error_table():
        ...
        df_orig = expected_error_table(N, prevalence, max_diff=4, max_error=max_err, correct=False)
        df_corr = expected_error_table(N, prevalence, max_diff=4, max_error=max_err, correct=True)
