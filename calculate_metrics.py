...
def sweep_metrics_precomp_fast(dir_WAs, max_diff, start=50, stop=150, step=10, diff=2, **kwargs):
    N=start
    ls_names_met=['Method', 'Mean experiments', 'Max samples per pool', 'N pools', 'Percentage check', 'Mean extra experiments', 'Mean steps']
            ...
            while diff<=max_diff:
                ...
                if diff==1:
                    ...
                    for fname in filenames:
                        fdir=os.path.join(WApath,fname)
                        WA=np.genfromtxt(fdir, delimiter=",")
                        mean_exp, extra_exp,  _, perc_check= mean_metrics_fast(well_assigner=WA, differentiate=diff, **kwargs)
                        n_wells=WA.shape[1]
                        M_exp=np.round(mean_exp, 2)
                        max_comp=np.max(np.sum(WA, axis=0))
                        method=re.sub('^WA_', '', fname)
                    ...
                    for fname in filenames:
                        ...
                        fdir=os.path.join(WApath,fname)
                        WA=np.genfromtxt(fdir, delimiter=",")
                        mean_exp, extra_exp,  _, perc_check= mean_metrics_fast(well_assigner=WA, 
                                                                                  differentiate=diff, **kwargs)
                        n_wells=WA.shape[1]
                        M_exp=np.round(mean_exp, 2)
                        max_comp=np.max(np.sum(WA, axis=0))
...
sweep_metrics_precomp_fast(**dict_kwargs)
