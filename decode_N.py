...
parser.add_argument('--save_grid_decoders', type=str, default='False', help='Save one decoder per grid element for each readout (forces continuous mode for CSV readouts).')
...
if readout_in.lower().endswith('csv'):
    ...
    if isinstance(df_out, pd.DataFrame) and not df_out.empty and save_grid_decoders and "grid_search_row_decoders" in df_out.columns:
        ...
        if len(decoder_rows) > 0:
            pd.DataFrame(decoder_rows).to_csv("decoded_grid_decoders.csv", index=False)
    ...
    if isinstance(df_out, pd.DataFrame) and not df_out.empty:
        ...
        if "grid_search_all_fits" in df_with_index.columns:
            df_with_index["grid_search_all_fits"] = df_with_index["grid_search_all_fits"].apply(
    ...
    decoded_csv = "decoded_grid.csv" if (grid_search or save_grid_decoders) else "decoded_readouts.csv"
    if isinstance(df_out, pd.DataFrame) and not df_out.empty:
