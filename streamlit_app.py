...
def render_design():
    ...
    if not params:
        note("Set your parameters above and select <b>Benchmark designs</b>. "
             ...
             f"D ≤ {MAX_DIFFERENTIATE}; anything outside that range is evaluated "
             "on the fly, which might take longer to compute.")
        return
...
def _render_summary(summary: pd.DataFrame, n_samp: int, diff: int, sens: float, spec: float):
    ...
    st.markdown(
        ...
        f'Column definitions are in the Guide.'
...
PAGE_BLURB = {
    ...
    "Prevalence": "Choose pooling parameters",
